...
describe("Validation", () => {
  it.each([
    ...
    expect(_ => validate(argsSchema, resSchema, fun, a1, a2, a3)).not.toThrow();
  });
  ...
  it.each([
    ...
    expect(_ => validate(argsSchema, resSchema, fun, a1, a2, a3)).toThrow(expected);
  });

  it("Should fail for too few arguments", () => {
    expect(_ => validate(argsSchema, resSchema, fun, 0, ["2"])).toThrow(/invalid argument/i);
  });

  it("Should fail for too many arguments", () => {
    expect(_ => validate(argsSchema, resSchema, fun, 0, ["2"], {}, 0)).toThrow(/invalid argument/i);
  });
});
...
describe("Validation with object output", () => {
  ...
  it.each([[{}], [Immutable.fromJS({})]])("should be successful for %s", arg => {
    expect(_ => validate(argOjectSchema, resOjectSchema, arg => arg, arg)).not.toThrow();
  });
});
